...
const loadBlockchain = async () => {
  ...
  const contractInstance = new web3.eth.Contract(AUCTIONABI, AUCTIONADDRESS, {
    from: accounts[0],
  });
...
export const auctionContractHandler =
  (_tokenId: string) => async (dispatch: any) => {
    ...
    contractInstance.methods
      ...
      .call()
      .then((res: any) => {
        if (res < 10000000000) res *= 1000; // convert to milliseconds (Epoch is usually expressed in seconds, but Javascript uses Milliseconds)
...
export const auctionStartAction =
  (_tokenId: string, _auctionTimeEnd: any, _objectId: string) =>
  async (dispatch: any) => {
    const { contractInstance, accounts } = await loadBlockchain();
...
export const bidHandlerAction =
  (_tokenId: string, _price: any) => async (dispatch: any) => {
    const { contractInstance, accounts } = await loadBlockchain();
  ...
  contractInstance.methods
    ...
    .on("receipt", (receipt: any) => {
      ...
      if (receipt.events.WithdrawPendingReturns.returnValues["isSuccess"]) {
        let amount =
          receipt.events.WithdrawPendingReturns.returnValues["amount"];
        console.log(amount);
        alert(
